...
namespace interface_bignumber {
	...
	const char* BigFloat::get_boolean_string(bool value) const {
		if (value) {
			return "True";
		}
		else {
	...
	std::ostream& interface_bignumber::operator << (std::ostream& out, const BigFloat& big_float) {
	...
	std::istream& interface_bignumber::operator >> (std::istream& in, BigFloat& big_float) {
